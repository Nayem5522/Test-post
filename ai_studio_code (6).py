...
async def save_channel(user_id: int, channel_id: int, channel_title: str):
    if not await ensure_bot_admin_rights(app, channel_id):
        raise ValueError("Bot must be an admin in the channel with 'Post Messages' and 'Edit Messages' permissions.")
...
async def shorten_link(user_id: int, long_url: str):
    ...
    try:
        ...
        if data.get("status") == "success" and data.get("shortenedUrl"):
            ...
            logger.warning(f"Shortener API for user {user_id} returned non-success status: {data.get('message', 'No message')}")
            return long_url
    except requests.exceptions.RequestException as e:
        logger.error(f"Shortener API request failed for user {user_id}: {e}")
    except Exception as e:
        logger.error(f"An unexpected error occurred in shorten_link for user {user_id}: {e}")
    return long_url
...
async def start_handler(bot, msg: Message):
    ...
    buttons = [
        [InlineKeyboardButton("🎬 Create Post (Help)", callback_data="create_post_help")],
    ...
    await msg.reply_photo(
        ...
        caption=(
            ...
            "**My Features:**\n"
            "✅ Automatically fetch details from TMDB.\n"
            "🖼️ Generate smart posters with watermarks and badges.\n"
            "🔗 Integrate with link shorteners.\n"

            "👍 Add like/love reaction buttons to posts.\n"
...
# ---------------------------------------------------------------------------
# 🔹 Post Creation Flow (Triggered by any text)
# ---------------------------------------------------------------------------
# List of all commands to exclude from this handler
ALL_COMMANDS = [
    "start", "post", "addchannel", "mychannels", "delchannel",
    "setcap", "delcap", "seecap", "addbutton", "mybuttons", "delbutton", "clearbuttons",
...
async def post_command_handler(bot, msg: Message):
    # This handler now takes any text that is not a command
    if msg.from_user.id in user_conversations and user_conversations[msg.from_user.id].get("state") != 'awaiting_forward_for_post':
...
async def select_post_callback(bot, cq: CallbackQuery):
    try:
        ...
        if len(parts) < 4: raise ValueError("Invalid callback data format")
    ...
    await cq.message.edit_text(
        ...
        f"💬 If you want the post in a language other than `{details.get('original_language')}`, enter the language name (e.g., Bengali, Hindi).\n\n"
        f"Otherwise, type **skip** to use the default."
    )
...
async def generate_final_post_preview(bot, uid, chat_id, status_msg: Message):
    ...
    # Store the final post data in the conversation
    convo['final_post'] = {
        'caption': caption,
    ...
    saved_channels = [ch for ch in user_data.get('channels', []) if await ensure_bot_admin_rights(bot, ch['id'])]
    ...
    if saved_channels:
        ...
        # New logic: No channels, instruct user to forward
...
async def generate_channel_caption(convo: dict, user_data: dict):
    ...
    if custom_caption: final_caption_parts.append(custom_caption)
    final_caption_parts.append(footer)

    return "\n\n".join(final_caption_parts)

async def post_to_channel(bot: Client, user_id: int, channel_id: int, reply_msg: Message):
    convo = user_conversations.get(user_id)
    if not convo or 'final_post' not in convo:
        if reply_msg: await reply_msg.edit_text("❌ Session expired! Please start over.")
        return

    await reply_msg.edit_text("⏳ Posting to channel...")
    ...
    try:
        ...
        await reply_msg.edit_text(f"✅ **Successfully posted to '{chat.title}'!**")
    except Exception as e:
        logger.error(f"Failed to post to channel {channel_id} for user {user_id}. Error: {e}")
        await reply_msg.edit_text(f"❌ **Failed to post to '{channel_id}'.**\n\n**Error:** `{e}`")
    finally:
...
async def direct_media_handler(bot, msg: Message):
    ...
    if not user or not user.get("channels"):
        return await msg.reply_text("⚠️ You haven't added any channels yet. Use `/addchannel` first.")

    buttons = [[InlineKeyboardButton(ch["title"], callback_data=f"sendto_{msg.id}_{ch['id']}")] 
               for ch in user["channels"] if await ensure_bot_admin_rights(bot, ch['id'])]

    if not buttons:
        return await msg.reply_text("⚠️ I don't have posting permissions in any of your saved channels. Please make me an admin.")

    await msg.reply_text("📤 **Choose a channel to post to:**", reply_markup=InlineKeyboardMarkup(buttons))
...
async def direct_media_post_callback(bot, cq: CallbackQuery):
    ...
    if not await ensure_bot_admin_rights(bot, channel_id):
        return await cq.answer("❌ Bot is not an admin or lacks 'Post Messages' permission!", show_alert=True)
    ...
    try:
        ...
        final_caption = (media_msg.caption.html if media_msg.caption else "") + \
                        ("\n\n" + user_data.get("custom_caption", "")) + \
                        "\n\n✨ **Posted by:** @Post_Generator_PrimeXBot"

        all_buttons = [[InlineKeyboardButton("👍 0", callback_data=f"react_DUMMY_like"), InlineKeyboardButton("❤️ 0", callback_data=f"react_DUMMY_love")]]
        for btn in user_data.get("custom_buttons", []):
        ...
        # Update reaction buttons with correct message ID
        ...
        await cq.message.edit_text("✅ Successfully posted!")
    except Exception as e:
        logger.error(f"Failed to post direct media: {e}")
        await cq.message.edit_text(f"❌ Failed to post. Error: {e}")
...
async def forward_handler(bot, msg: Message):
    ...
    # Check if the user is waiting to post a generated message
    convo = user_conversations.get(uid)
    is_awaiting_post = convo and convo.get('state') == 'awaiting_forward_for_post'
    
    status_msg = await msg.reply_text("⏳ Processing channel...")

    try:
        saved, status_text = await save_channel(uid, channel.id, channel.title)
        if saved:
            await status_msg.edit_text(f"✅ Channel **{channel.title}** added successfully!")
            # If the user was waiting to post, post it now to the new channel
            if is_awaiting_post:
                await post_to_channel(bot, uid, channel.id, status_msg)
        else:
            await status_msg.edit_text(f"⚠️ This channel is already in your list.")
            # If already exists and waiting to post, post it
            if is_awaiting_post:
                await post_to_channel(bot, uid, channel.id, status_msg)

    except ValueError as e:
        await status_msg.edit_text(f"❌ Failed to add channel: {e}")
    except Exception as e:
...
async def reaction_handler(bot, cq: CallbackQuery):
    try:
        ...
        post = await reactions_collection.find_one_and_update(
            {"message_id": msg_id},
            {"$setOnInsert": {"reactions": {"like": [], "love": []}}},
            upsert=True, return_document=True
        )
        
        # Logic to toggle reaction
        other_reaction = "love" if reaction == "like" else "like"
        if user_id in post["reactions"].get(reaction, []):
            post["reactions"][reaction].remove(user_id) # Un-react
        else:
            if user_id in post["reactions"].get(other_reaction, []):
                post["reactions"][other_reaction].remove(user_id) # Switch reaction
            post["reactions"].setdefault(reaction, []).append(user_id) # Add new reaction
...
async def navigation_handler(bot, cq: CallbackQuery):
    ...
    if data == "start_menu":
        ...
        help_text = (
            "📚 **Help Menu**\n\n"
            "**Post Creation:**\n"
            "🔹 `Movie/Series Name` - Send a name to start creating a post.\n"
            "   (Example: `Avatar 2009`)\n"
            "🔹 **Send Photo/Video** - To create a simple post directly.\n\n"
            "**Channel Management:**\n"
            ...
            "🔹 `/delcap` - Delete the custom caption.\n"
            "🔹 `/addbutton <name | link>` - Add a custom button.\n"
            "🔹 `/clearbuttons` - Delete all custom buttons.\n"
        ...
        settings_text = (
            ...
            "Use the `/help` command to see how to change these settings."
        )
...
async def refresh_callback(bot, cq: CallbackQuery):
    if await is_subscribed(bot, cq.from_user.id):
        ...
        # Create a mock message object to pass to start_handler
        mock_msg = type('MockMsg', (), {'from_user': cq.from_user, 'chat': cq.message.chat, 'reply_photo': cq.message.reply_photo, 'reply_text': cq.message.reply_text})()
        await start_handler(bot, mock_msg)
    else:
...
async def button_commands(bot: Client, msg: Message):
    ...
    if command == "addbutton":
        ...
        except (ValueError, IndexError): await msg.reply_text("⚠️ Invalid format.")
...
async def settings_commands(bot: Client, msg: Message):
    ...
    if len(msg.command) > 1:
        ...
        elif command == "badge": user_conversations.setdefault(user_id, {})['temp_badge_text'] = value; await msg.reply_text(f"✅ Badge for the next post set to: `{value}`")
    else:
        if command == "settings": await navigation_handler(bot, type('MockCQ', (), {'data': 'settings_menu', 'from_user': msg.from_user, 'message': msg, 'answer': (lambda: None)})()); return
        ...
        elif command == "badge": user_conversations.get(user_id, {}).pop('temp_badge_text', None); await msg.reply_text("🗑️ Temporary badge text removed.")
