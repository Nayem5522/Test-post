...
def download_cascade():
    ...
    if not os.path.exists(cascade_file):
        ...
        try:
            ...
            logger.error(f"Could not download cascade file. Face detection disabled. Error: {e}")
            return None
    return cascade_file
...
def get_media_details(media_type: str, media_id: int):
    ...
    try:
        ...
        logger.error(f"TMDB Details Fetch Error: {e}"); return None
...
def create_poster(poster_url: str, watermark_text: str, badge_text: str = None):
    ...
    try:
        ...
        logger.error(f"Image processing error: {e}"); return None, f"Image processing error: {e}"
...
async def shorten_link(user_id: int, long_url: str):
    ...
    try:
        ...
        logger.error(f"Link shortener API error for user {user_id}: {e}"); return long_url
...
async def start_handler(bot, msg: Message):
    if not await is_subscribed(bot, msg.from_user.id):
        try:
            ...
            logger.error(f"Could not get invite link for {AUTH_CHANNEL}: {e}"); await msg.reply_text("Could not get channel link. Contact owner.")
        return

    main_menu_buttons = [
        [InlineKeyboardButton("📝 Generate Post", callback_data="generate_post_help"), InlineKeyboardButton("⚙️ Settings", callback_data="settings_menu")],
...
async def help_command_handler(bot, msg: Message):
    await msg.reply_text(
        ("📚 **Help Guide**\n\n"
         ...
         "• `/settings`: Access all other settings.")
    )

@app.on_message(filters.private & filters.command("settings"))
async def settings_command_handler(bot, msg: Message):
    await msg.reply_text("Please use the inline settings menu.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Open Settings", callback_data="settings_menu")]]))
...
async def add_channel_cmd(bot, msg: Message):
    ...
    try:
        if await users_db.find_one({"user_id": msg.from_user.id, "channels.id": channel_id}):
            return await msg.reply_text("This channel is already in your list.")
        if not await ensure_bot_admin_rights(bot, channel_id):
            return await msg.reply_text("I need 'Post Messages' and 'Edit Messages' admin rights in that channel.")
        ...
        logger.error(f"Error adding channel {channel_id} for user {msg.from_user.id}: {e}"); await msg.reply_text("An unexpected error occurred.")
...
async def set_cap_cmd(bot, msg: Message):
    ...
    await users_db.update_one({"user_id": msg.from_user.id}, {"$set": {"custom_caption": caption}}, upsert=True)
    await msg.reply_text("✅ Custom caption set!")
...
async def del_cap_cmd(bot, msg: Message):
    await users_db.update_one({"user_id": msg.from_user.id}, {"$unset": {"custom_caption": ""}})
...
async def clear_buttons_cmd(bot, msg: Message):
    await users_db.update_one({"user_id": msg.from_user.id}, {"$set": {"custom_buttons": []}})
...
async def stats_handler(bot, msg: Message):
    ...
    total_channels = await users_db.aggregate([{"$project": {"channel_count": {"$ifNull": [{"$size": "$channels"}, 0]}}}, {"$group": {"_id": None, "total": {"$sum": "$channel_count"}}}]).to_list(1)
    total_channels = total_channels[0]['total'] if total_channels else 0
...
async def forward_handler(bot, msg: Message):
    if not msg.forward_from_chat or msg.forward_from_chat.type != enums.ChatType.CHANNEL:
        return await msg.reply_text("Please forward from a public or private channel.")
    channel = msg.forward_from_chat
    try:
        if await users_db.find_one({"user_id": msg.from_user.id, "channels.id": channel.id}):
            return await msg.reply_text("This channel is already in your list.")
        if not await ensure_bot_admin_rights(bot, channel.id):
            return await msg.reply_text(f"I need admin rights in **{channel.title}** to post messages.")
        ...
        logger.error(f"Error saving forwarded channel {channel.id} for {msg.from_user.id}: {e}")
        await msg.reply_text("An error occurred while saving the channel.")

@app.on_message(filters.private & (filters.photo | filters.video))
async def legacy_media_handler(bot, msg: Message):
    settings = await get_user_settings(msg.from_user.id)
    if not (channels := settings.get("channels")):
        return await msg.reply_text("You have no channels set. Use `/addchannel` first.")
...
async def conversation_handler(bot, msg: Message):
    ...
    async def process_link(quality, next_state, next_prompt):
        ...
        await msg.reply_text(f"✅ Link added. Now, {next_prompt}")

    if state.startswith("set_"):
        setting_key = state.split("_", 1)[1]
        if text.lower() == "remove":
            await users_db.update_one({"user_id": user_id}, {"$unset": {setting_key: ""}})
            await msg.reply_text(f"✅ {setting_key.replace('_', ' ').title()} has been removed.")
        else:
            await users_db.update_one({"user_id": user_id}, {"$set": {setting_key: text}}, upsert=True)
            await msg.reply_text(f"✅ {setting_key.replace('_', ' ').title()} has been set.")
        ...
        pr_msg = await msg.reply_text("✅ Data collection complete! Generating preview...")
        await generate_final_post_preview(bot, user_id, pr_msg)
        ...
        if text.lower() == 'done':
            ...
            pr_msg = await msg.reply_text("✅ All season data collected! Generating preview...")
            await generate_final_post_preview(bot, user_id, pr_msg)
        elif text.isdigit() and int(text) > 0:
            session['current_season'] = text; session['state'] = 'wait_season_link'; await msg.reply_text(f"OK. Send download link for **Season {text}**.")
...
async def generate_final_post_preview(bot, user_id, pr_msg: Message):
    ...
    settings = await get_user_settings(user_id)
    badge_text = session.pop('badge_text', None) # Use badge and remove it
    ...
    if channels := settings.get("channels"):
        ...
        await bot.send_message(user_id, "✅ Preview generated. No channels saved.", reply_to_message_id=preview_msg.id)
...
async def main_callback_router(bot, cq: CallbackQuery):
    ...
    if data == "main_menu":
        await cq.answer(); await start_handler(bot, cq.message) # Simplified back
        ...
        settings = await get_user_settings(user_id)
        settings_buttons = [
            [InlineKeyboardButton(f"Watermark: {'✅' if settings.get('watermark_text') else '❌'}", callback_data="set_watermark_text")],
            [InlineKeyboardButton(f"Shortener API: {'✅' if settings.get('shortener_api') else '❌'}", callback_data="set_shortener_api"), InlineKeyboardButton(f"Shortener URL: {'✅' if settings.get('shortener_url') else '❌'}", callback_data="set_shortener_url")],
            [InlineKeyboardButton(f"Tutorial Link: {'✅' if settings.get('tutorial_link') else '❌'}", callback_data="set_tutorial_link")],
            [InlineKeyboardButton("⌫ Back to Main Menu", callback_data="main_menu")]
        ]
        await cq.message.edit_caption(caption="⚙️ **Bot Settings**", reply_markup=InlineKeyboardMarkup(settings_buttons))
        ...
        try:
            ...
            media_msg = await bot.get_messages(user_id, last_media_id)
            ...
            # Add reactions and custom buttons
            ...
            final_markup = InlineKeyboardMarkup([reaction_row] + buttons)
            await sent_msg.edit_reply_markup(final_markup)
            ...
            logger.error(f"Failed to post legacy media: {e}"); await cq.answer("❌ Failed to post!", show_alert=True)
        ...
        try:
            ...
            logger.error(f"Failed to post to channel {channel_id}: {e}"); await cq.message.edit_text(f"❌ **Failed to post.**\nError: `{e}`")
        finally:
        ...
        # Rebuild keyboard preserving custom buttons
        ...
        new_markup = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"👍 {like_count}", callback_data=f"react_{msg_id}_{chat_id}_like"),
             InlineKeyboardButton(f"❤️ {love_count}", callback_data=f"react_{msg_id}_{chat_id}_love")]
        ] + other_buttons)
        try: await cq.message.edit_reply_markup(new_markup)
        except Exception: pass # Ignore errors if message is not modified
